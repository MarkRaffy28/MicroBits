...
const StatCard = ({ icon, label, value, sub, color, onClick, chart, chartColor }) => (
  <div
    onClick={onClick}
    className={`bg-gray-800 rounded-xl p-5 border border-gray-700/50 flex flex-col gap-3 ${
      onClick ? "cursor-pointer hover:border-blue-500/50 transition-colors duration-200" : ""
    }`}
  >
...
const StatusPill = ({ status }) => (
  <span className={`inline-block px-2 py-0.5 rounded text-xs text-white ${STATUS_COLORS[status] ?? "bg-gray-600"}`}>
...
function Dashboard() {
  ...
  /* ─── Order status ratio ─── */
  const statusRatioKeys = ["to_pay", "to_ship", "to_receive", "completed", "cancelled", "return_refund"];
  const statusRatioColors = {
  ...
  const orderStatusRatio = orders.length
    ? statusRatioKeys
        ...
        .map((k) => ({
          ...
          color: statusRatioColors[k],
          pct:   Math.round((statusCounts[k] / orders.length) * 100),
        }))
    : [];
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon="bi-receipt"         label="Total Orders"
          value={orders.length}     sub={`${statusCounts.to_pay} to pay · ${statusCounts.to_ship} to ship`}
          color="text-blue-400"     chart={last7}  chartColor="bg-blue-500"
          onClick={() => navigate("/admin/orders/all")}
        />
        <StatCard
          icon="bi-currency-dollar" label="Revenue"
        ...
        <StatCard
          ...
          value={products.length}   sub={`${lowStock.length} low stock`}
          color="text-orange-400"   chart={stockChart} chartColor="bg-orange-500"
          onClick={() => navigate("/admin/products")}
        />
        <StatCard
          icon="bi-people"          label="Users"
          value={users.length}      sub={`${users.filter((u) => u.role === "admin").length} admins`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700/50">
          ...
          <div className="mt-5 pt-4 border-t border-gray-700 grid grid-cols-3 gap-2 text-center">
            {[
              { label: "To Pay",    value: statusCounts.to_pay,     color: "text-yellow-400" },
              { label: "To Ship",   value: statusCounts.to_ship,    color: "text-blue-400" },
              { label: "Completed", value: statusCounts.completed,  color: "text-green-400" },
                ...
                <tbody className="divide-y divide-gray-700/50">
                  {recentOrders.map((o) => (
                    <tr key={o.id}
                      ...
                      <td className="py-2 pr-3 text-gray-400 font-mono text-xs truncate max-w-[80px]" title={o.id}>
                        #{o.id}
                      </td>
                      <td className="py-2 pr-3 text-white font-medium">{getFullName(o.userId)}</td>
                      <td className="py-2 pr-3 text-green-400 font-semibold">${o.totalAmount?.toFixed(2) ?? "0.00"}</td>
